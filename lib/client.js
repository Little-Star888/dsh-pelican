...
`
		...
		function PelicanRide() {
			...
			React.useEffect(function () {
				const tickId = setInterval(function tick() {
					...
					if (flapRef.current) {
						...
						// 近翼：肩 -20°~+20° 主摆动，肘 +10° 滞后，翼 +8° 更滞后，羽毛 +6° 甩动
						nearShoulderRef.current.setAttribute('transform', 'translate(185 162) rotate(' + (-s1 * 20).toFixed(1) + ')')
						nearElbowRef.current.setAttribute('transform', 'rotate(' + (s2 * 14).toFixed(1) + ')')
						nearWingRef.current.setAttribute('transform', 'translate(26 0) rotate(' + (s3 * 12).toFixed(1) + ')')
						nearFeatherRef.current.setAttribute('transform', 'translate(24 0) rotate(' + (s4 * 8).toFixed(1) + ')')
						// 远翼：反相（s1 取负），幅度更小，透视收敛
						farShoulderRef.current.setAttribute('transform', 'translate(183 163) rotate(' + (s1 * 14).toFixed(1) + ')')
						farElbowRef.current.setAttribute('transform', 'rotate(' + (-s2 * 10).toFixed(1) + ')')
						farWingRef2.current.setAttribute('transform', 'translate(22 0) rotate(' + (-s3 * 8).toFixed(1) + ')')
						farFeatherRef.current.setAttribute('transform', 'translate(20 0) rotate(' + (-s4 * 5).toFixed(1) + ')')
					} else {
						// 静止：所有关节复位
						nearShoulderRef.current.setAttribute('transform', 'translate(185 162) rotate(0)')
						nearElbowRef.current.setAttribute('transform', 'rotate(0)')
						nearWingRef.current.setAttribute('transform', 'translate(26 0) rotate(0)')
						nearFeatherRef.current.setAttribute('transform', 'translate(24 0) rotate(0)')
						farShoulderRef.current.setAttribute('transform', 'translate(183 163) rotate(0)')
						farElbowRef.current.setAttribute('transform', 'rotate(0)')
						farWingRef2.current.setAttribute('transform', 'translate(22 0) rotate(0)')
						farFeatherRef.current.setAttribute('transform', 'translate(20 0) rotate(0)')
					}
				}, 33)
			...
			const scene = el('div', { className: 'dsh-pel-scene' },
				...
				// 同款关节链，但幅度更小、颜色更深（透视收敛），与近翼反相增强层次。
				el('svg', { ref: farWingRef, className: 'dsh-pel-wing dsh-pel-wing-far', viewBox: '0 0 460 330', 'aria-hidden': true },
					el('g', { ref: farShoulderRef, transform: 'translate(183 163)' },
						el('g', { ref: farElbowRef, transform: 'translate(0 0)' },
							el('path', { d: 'M 0 0 L 18 -9 L 26 -1 L 18 8 Z', fill: '#c3d9ec', stroke: '#a9c4dc', strokeWidth: 1, opacity: 0.85 }),
							el('path', { d: 'M 1 -2 L 15 -8 L 22 -1 L 17 6 Z', fill: '#b7d0e4', stroke: '#9fbcd4', strokeWidth: 1, opacity: 0.85 }),
							el('g', { ref: farWingRef2, transform: 'translate(22 0)' },
								el('path', { d: 'M 0 0 L 15 -5 L 22 0 L 15 6 Z', fill: '#c3d9ec', stroke: '#a9c4dc', strokeWidth: 1, opacity: 0.85 }),
								el('g', { ref: farFeatherRef, transform: 'translate(20 0)' },
									el('path', { d: 'M 0 0 Q 12 -10 24 -5 Q 16 0 0 0 Z', fill: '#b7d0e4', stroke: '#9fbcd4', strokeWidth: 1, opacity: 0.85 }),
									el('path', { d: 'M 1 1 Q 15 -3 26 0 Q 17 4 1 1 Z', fill: '#a9c4dc', stroke: '#8fb0cb', strokeWidth: 1, opacity: 0.85 })
								)
							)
						)
					)
				),
				// 近翼层：位于身体上方（z-index 3）。真实鸟翼是多关节链——肩节 + 前臂节 +
				// 翼尖节，拍翅时翼尖相位滞后、翅膀自然弯曲，形成弧线摆动（这才是"活的"）。
				el('svg', { ref: wingRef, className: 'dsh-pel-wing dsh-pel-wing-near', viewBox: '0 0 460 330', 'aria-hidden': true },
					el('g', { ref: nearShoulderRef, transform: 'translate(185 162)' },
						el('g', { ref: nearElbowRef, transform: 'translate(0 0)' },
							// 覆羽（贴近身体，3 片叠瓦）
							el('path', { d: 'M 0 0 L 22 -10 L 30 0 L 22 10 Z', fill: '#e2eef9', stroke: '#b9cfe2', strokeWidth: 1 }),
							el('path', { d: 'M 2 -2 L 18 -8 L 26 2 L 20 8 Z', fill: '#d5e7f5', stroke: '#b3c9de', strokeWidth: 1 }),
							el('path', { d: 'M 3 -4 L 16 -7 L 22 3 L 17 7 Z', fill: '#c9e0f0', stroke: '#aec8de', strokeWidth: 1 }),
							el('g', { ref: nearWingRef, transform: 'translate(26 0)' },
								// 前臂
								el('path', { d: 'M 0 0 L 18 -6 L 26 0 L 18 7 Z', fill: '#dcebf7', stroke: '#b9cfe2', strokeWidth: 1 }),
								// 初级飞羽（翼尖，3 根，扇形展开）
								el('g', { ref: nearFeatherRef, transform: 'translate(24 0)' },
									el('path', { d: 'M 0 0 Q 14 -12 28 -6 Q 18 0 0 0 Z', fill: '#cfe3f3', stroke: '#aec8de', strokeWidth: 1 }),
									el('path', { d: 'M 2 1 Q 17 -5 30 0 Q 20 5 2 1 Z', fill: '#c3d9ec', stroke: '#a4bfd7', strokeWidth: 1 }),
									el('path', { d: 'M 2 2 Q 15 4 26 9 Q 16 11 2 2 Z', fill: '#bcd3e8', stroke: '#a4bfd7', strokeWidth: 1 })
								)
							)
						)
					)
