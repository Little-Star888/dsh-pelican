...
.dsh-pel-card svg.dsh-pel-scene-main { width: 296px; height: auto; border-radius: 12px; display: block; }
.dsh-pel-scene { position: relative; }
/* 独立翅膀层：与场景同 viewBox（坐标天然对齐），远翼在身体下、近翼在身体上 */
.dsh-pel-wing { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; overflow: visible; }
.dsh-pel-wing-far { z-index: 1; }
.dsh-pel-wing-near { z-index: 3; }
...
`
		...
		function PelicanRide() {
			...
			// 关节链引用：肩 / 肘 / 翼 / 翼尖羽毛（近翼与远翼）
			const nearShoulderRef = React.useRef(null)
			const nearElbowRef = React.useRef(null)
			const nearWingRef = React.useRef(null)
			const nearFeatherRef = React.useRef(null)
			const farShoulderRef = React.useRef(null)
			const farElbowRef = React.useRef(null)
			const farWingRef2 = React.useRef(null)
			const farFeatherRef = React.useRef(null)
			const nearShadeRef = React.useRef(null)
			const farShadeRef = React.useRef(null)
			...
			React.useEffect(function () {
				const tickId = setInterval(function tick() {
					...
					if (flapRef.current) {
						...
						// 关节链拍翅（真实鸟翼运动学）：
						// 肩节绕肩关节做主摆动（翼尖在屏幕平面内画弧），肘节、翼节、翼尖羽毛
						// 依次相位滞后 → 翅膀像鞭子一样自然弯曲，而不是刚体翻转。
						const ph = flapPhaseRef.current
						const s1 = Math.sin(ph)
						const s2 = Math.sin(ph - 0.55)
						const s3 = Math.sin(ph - 1.05)
						const s4 = Math.sin(ph - 1.5)
						// 近翼：肩 ±16° 主摆动，翼尖绕肩在屏幕平面内上下画弧（避开头/喙）
						nearShoulderRef.current.setAttribute('transform', 'translate(188 158) rotate(' + (-s1 * 16).toFixed(1) + ')')
						nearElbowRef.current.setAttribute('transform', 'rotate(' + (s2 * 6).toFixed(1) + ')')
						nearWingRef.current.setAttribute('transform', 'translate(48 -4) rotate(' + (s3 * 8).toFixed(1) + ')')
						nearFeatherRef.current.setAttribute('transform', 'rotate(' + (s4 * 6).toFixed(1) + ')')
						// 翼面明暗：摆动到上/下极限时翼面转侧，变深强化体积（0→0.28）
						nearShadeRef.current.setAttribute('opacity', (Math.abs(s1) * 0.28).toFixed(3))
						// 远翼：反相（s1 取负），幅度更小，透视收敛
						farShoulderRef.current.setAttribute('transform', 'translate(186 160) rotate(' + (s1 * 12).toFixed(1) + ')')
						farElbowRef.current.setAttribute('transform', 'rotate(' + (-s2 * 4).toFixed(1) + ')')
						farWingRef2.current.setAttribute('transform', 'translate(36 -3) rotate(' + (-s3 * 6).toFixed(1) + ')')
						farFeatherRef.current.setAttribute('transform', 'rotate(' + (-s4 * 4).toFixed(1) + ')')
						farShadeRef.current.setAttribute('opacity', (Math.abs(s1) * 0.24).toFixed(3))
					} else {
						// 静止：所有关节复位
						nearShoulderRef.current.setAttribute('transform', 'translate(188 158) rotate(0)')
						nearElbowRef.current.setAttribute('transform', 'rotate(0)')
						nearWingRef.current.setAttribute('transform', 'translate(48 -4) rotate(0)')
						nearFeatherRef.current.setAttribute('transform', 'rotate(0)')
						nearShadeRef.current.setAttribute('opacity', '0')
						farShoulderRef.current.setAttribute('transform', 'translate(186 160) rotate(0)')
						farElbowRef.current.setAttribute('transform', 'rotate(0)')
						farWingRef2.current.setAttribute('transform', 'translate(36 -3) rotate(0)')
						farFeatherRef.current.setAttribute('transform', 'rotate(0)')
						farShadeRef.current.setAttribute('opacity', '0')
					}
				}, 33)
			...
			const scene = el('div', { className: 'dsh-pel-scene' },
				el('svg', {
					className: 'dsh-pel-scene-main',
					viewBox: '0 0 460 330',
					role: 'img',
					'aria-label': '鹈鹕环海骑行动画',
				},
				...
				// 远翼层：位于身体下方（z-index 1），只露翼尖——符合"远翼在后"的透视。
				// 同款向右展开的宽弧翼面，但更小、颜色更深（透视收敛），与近翼反相增强层次。
				el('svg', { ref: farWingRef, className: 'dsh-pel-wing dsh-pel-wing-far', viewBox: '0 0 460 330', 'aria-hidden': true },
					el('g', { ref: farShoulderRef, transform: 'translate(186 160)' },
						el('g', { ref: farElbowRef, transform: 'translate(0 0)' },
							el('path', { d: 'M 0 0 C 8 -6 18 -9 26 -8 C 22 -2 12 1 5 1 C 2 2 0 1 0 0 Z', fill: '#c3d9ec', stroke: '#a9c4dc', strokeWidth: 1, opacity: 0.85 }),
							el('path', {
								d: 'M 2 -2 C 12 -9 24 -11 36 -9 C 40 -8 42 -5 41 -2 C 37 3 26 7 14 7 C 7 7 2 4 2 -2 Z',
								fill: '#b7d0e4', stroke: '#9fbcd4', strokeWidth: 1.2, opacity: 0.85
							}),
							el('path', { d: 'M 8 -7 C 18 -10 30 -10 38 -7', fill: 'none', stroke: '#a9c4dc', strokeWidth: 1, opacity: 0.6 }),
							el('g', { ref: farWingRef2, transform: 'translate(36 -3)' },
								el('g', { ref: farFeatherRef, transform: 'translate(0 0)' },
									el('path', { d: 'M 0 0 Q 10 -9 20 -6 Q 11 -1 0 0 Z', fill: '#a9c4dc', stroke: '#8fb0cb', strokeWidth: 1, opacity: 0.85 }),
									el('path', { d: 'M 0 2 Q 11 -3 22 0 Q 12 4 0 2 Z', fill: '#9fbcd4', stroke: '#7fa6c2', strokeWidth: 1, opacity: 0.85 })
								)
							),
							el('path', { ref: farShadeRef, d: 'M 2 -2 C 12 -9 24 -11 36 -9 C 40 -8 42 -5 41 -2 C 37 3 26 7 14 7 C 7 7 2 4 2 -2 Z', fill: '#5c7e98', opacity: 0 })
						)
					)
				),
				// 近翼层：位于身体上方（z-index 3）。真实鸟翼从侧面看是向右展开的宽弧翼面，
				// 前缘略上翘、后缘弧回、翼尖带初级飞羽。拍翅时翼尖绕肩做小幅弧线上下扑动，
				// 翼尖略低于头部/喙（不穿模），配合翼面明暗变化强化立体感。
				el('svg', { ref: wingRef, className: 'dsh-pel-wing dsh-pel-wing-near', viewBox: '0 0 460 330', 'aria-hidden': true },
					el('g', { ref: nearShoulderRef, transform: 'translate(188 158)' },
						el('g', { ref: nearElbowRef, transform: 'translate(0 0)' },
							// 覆羽（贴肩，2 片叠瓦，亮色）
							el('path', { d: 'M 0 0 C 10 -8 22 -12 32 -10 C 28 -3 16 1 6 2 C 3 2 0 1 0 0 Z', fill: '#eef4fb', stroke: '#c4d6e6', strokeWidth: 1 }),
							el('path', { d: 'M 1 1 C 11 -4 21 -7 30 -5 C 24 1 14 4 5 4 C 2 4 1 2 1 1 Z', fill: '#e5f0fa', stroke: '#c4d6e6', strokeWidth: 1 }),
							// 主翼面（宽弧：前缘略上翘、翼尖圆润、后缘弧回，向右展开）
							el('path', {
								d: 'M 2 -3 C 14 -11 30 -14 46 -11 C 52 -10 55 -6 53 -2 C 48 4 34 9 18 9 C 9 9 2 6 2 -3 Z',
								fill: '#dcebf7', stroke: '#b9cfe2', strokeWidth: 1.4
							}),
							// 次级覆羽线（层次）
							el('path', { d: 'M 10 -8 C 24 -13 40 -13 50 -9', fill: 'none', stroke: '#b3c9de', strokeWidth: 1, opacity: 0.6 }),
							el('path', { d: 'M 12 -2 C 26 -6 42 -5 50 -2', fill: 'none', stroke: '#b3c9de', strokeWidth: 1, opacity: 0.6 }),
							// 初级飞羽（翼尖，3 根，向右扇形，略下倾）
							el('g', { ref: nearWingRef, transform: 'translate(48 -4)' },
								el('g', { ref: nearFeatherRef, transform: 'translate(0 0)' },
									el('path', { d: 'M 0 0 Q 12 -12 26 -8 Q 14 -2 0 0 Z', fill: '#cfe3f3', stroke: '#aec8de', strokeWidth: 1 }),
									el('path', { d: 'M 0 2 Q 14 -4 28 0 Q 15 5 0 2 Z', fill: '#c3d9ec', stroke: '#a4bfd7', strokeWidth: 1 }),
									el('path', { d: 'M 0 4 Q 12 3 24 7 Q 13 9 0 4 Z', fill: '#bcd3e8', stroke: '#a4bfd7', strokeWidth: 1 })
								)
							),
							// 翼面明暗层（拍翅时随摆动加深，模拟翼面转向，强化体积）
							el('path', { ref: nearShadeRef, d: 'M 2 -3 C 14 -11 30 -14 46 -11 C 52 -10 55 -6 53 -2 C 48 4 34 9 18 9 C 9 9 2 6 2 -3 Z', fill: '#7fa6c2', opacity: 0 })
						)
					)
				)
			)
			...
			// 自动倍速 + 自动扇翅：思考中蹬快点（3x）更像在努力，翅膀也随之扇动；
			// 回复结束后恢复 1x，翅膀停止扇动。
			const applyAutoSpeed = function (st) {
				...
				const wantFlap = st === 'thinking'
				if (flapRef.current !== wantFlap) {
					flapRef.current = wantFlap
					setFlapState(wantFlap)
				}
			}
