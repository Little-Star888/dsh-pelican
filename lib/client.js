		...
		function PelicanRide() {
			...
			React.useEffect(function () {
				const tickId = setInterval(function tick() {
					...
					if (flapRef.current) {
						...
						const fa = Math.sin(flapPhaseRef.current) * 38
						wingRef.current.setAttribute('transform', 'rotate(' + (-fa).toFixed(1) + ' 190 166)')
						farWingRef.current.setAttribute('transform', 'rotate(' + (-fa * 0.8).toFixed(1) + ' 184 162)')
					} else {
			...
			const scene = el('svg', {
				...
				el('g', { ref: farWingRef, transform: 'rotate(0 184 162)' },
					el('path', {
						d: 'M 178 158 C 192 152 202 158 202 168 C 202 176 192 180 184 178 C 178 176 174 170 176 162 C 176 160 177 158 178 158 Z',
				...
				el('g', { ref: wingRef, transform: 'rotate(0 190 166)' },
					el('path', {
						d: 'M 178 156 C 198 150 214 158 216 174 C 216 186 204 192 190 190 C 178 188 170 178 174 164 C 175 160 176 157 178 156 Z',
						fill: '#dcebf7', stroke: '#b9cfe2', strokeWidth: 1.4
					}),
					el('path', { d: 'M 182 162 C 194 158 206 164 210 172 M 178 170 C 190 166 202 172 206 179', fill: 'none', stroke: '#b3c9de', strokeWidth: 1.2, strokeLinecap: 'round' })
				),
				el('path', {
			...
			const speech = status === 'thinking'
				? el('div', { className: 'dsh-pel-speech' }, '我在呢，稍等～')
				: status === 'done'
		...
		function apply(ctx) {
			...
			ctx.slots.inject('shell.overlay', function () {
				return ctx.slots.register(
					{ name: 'shell.overlay', id: 'pelican-sea-ride' },
					PelicanRide
				)
			})
		}
