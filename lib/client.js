...
.dsh-pel-wing { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; overflow: visible; transform-box: view-box; }
.dsh-pel-wing-far { z-index: 1; transform-origin: 184px 162px; }
.dsh-pel-wing-near { z-index: 3; transform-origin: 190px 166px; }
...
`
		...
		function PelicanRide() {
			...
			React.useEffect(function () {
				const tickId = setInterval(function tick() {
					...
					if (flapRef.current) {
						...
						// 3D 拍翅：绕肩部水平轴（rotateX）翻转，配合 perspective 产生近大远小的
						// 真实纵深；rotateZ 让翼尖在屏幕内上下摆动。翼根被身体遮挡，翼尖翻进/翻出。
						const fa = Math.sin(flapPhaseRef.current)
						const px = 700
						const rotX = -fa * 46
						const rotZ = fa * 10
						wingRef.current.style.transform =
							'perspective(' + px + 'px) rotateX(' + rotX.toFixed(1) + 'deg) rotateZ(' + rotZ.toFixed(1) + 'deg)'
						// 远翼：相位略滞后、幅度更小（透视下更收敛），增强深度
						const fa2 = Math.sin(flapPhaseRef.current - 0.4)
						const rotX2 = -fa2 * 38
						const rotZ2 = fa2 * 7
						farWingRef.current.style.transform =
							'perspective(' + px + 'px) rotateX(' + rotX2.toFixed(1) + 'deg) rotateZ(' + rotZ2.toFixed(1) + 'deg)'
					} else {
						wingRef.current.style.transform = 'none'
						farWingRef.current.style.transform = 'none'
					}
				}, 33)
			...
			const scene = el('div', { className: 'dsh-pel-scene' },
				...
				// 远翼层：与场景同 viewBox，位于身体下方（z-index 1），只露翼尖——符合"远翼在后"的透视
